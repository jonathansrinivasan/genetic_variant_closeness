---
title: Convert Rdata to CSV file format
author: "Jonathan Srinivasan"
date: "January 2023"
output:
  html_document:
    theme: flatly
...
#h2dist<-read.csv(file = "data/Covariates/herg-correlation-analysis.csv", header = FALSE)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
write.csv(cln.d, file = "csv_data/scn5a_brs1_seq_data.csv")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
write.csv(herg.combined.data, file = "csv_data/herg_combined_data.csv")
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
write.csv(cln.d, file = "csv_data/clin_d.csv")
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}
write.csv(h2dist, file = "csv_data/h2dist.csv")
```
...
# d <- d[d$mut_type == "missense" & d$isoform=="A",]
...
# 
#load("All.Data.Funcdist.New.Rdata")
# load("Seq.Data.Funcdist2.New.RData")
# distseq<-cln.d
# load("Obs.Data.Funcdist.New.RData")
# distobs<-cln.d
# load("Obs.Max.Data.New.RData")
# maxobs<-cln.d
# load("Obs.Mean.Data.New.RData")
# meanobs<-cln.d
# load("all.Max.Data.New.RData")
# maxall<-cln.d
# load("all.Mean.Data.New.RData")
# meanall<-cln.d
# load("Seq.Observed.Data.New.Rdata")
# seqobs<-cln.d
# 
# distseq<-select(distseq,var, resnum, lqt2_dist, lqt2_dist_weight)
# distobs<-select(distobs,var, resnum, lqt2_dist, lqt2_dist_weight)
# maxobs<-select(maxobs,var, resnum, lqt2_dist, lqt2_dist_weight)
# meanobs<-select(meanobs,var, resnum, lqt2_dist, lqt2_dist_weight)
# maxall<-select(maxall,var, resnum, lqt2_dist, lqt2_dist_weight)
# meanall<-select(meanall,var, resnum, lqt2_dist, lqt2_dist_weight)
# seqobs<-select(seqobs,var, resnum, lqt2_dist, lqt2_dist_weight)
# 
# herg.combined.data<-left_join(herg.combined.data, distseq, by=c('resnum','var'), suffix= c('','_sequence'))
# herg.combined.data<-left_join(herg.combined.data, distobs, by=c('resnum', 'var'),  suffix= c('','_observed'))
...
# 
# save(herg.combined.data, file="all.lqt2.data.trunc.Rdata")
```
...
load('all.lqt2.data.trunc.Rdata')
herg.combined.data <- herg.combined.data[,!names(herg.combined.data) %in%  "cardiacboost"]
herg.combined.data <- unique(herg.combined.data)
results.1.data<-herg.combined.data[!is.na(herg.combined.data$provean_score) & !is.na(herg.combined.data$revel_score) & !is.na(herg.combined.data$pamscore),]
...
xList=c('hm_ssPeak','hm_tailPeak','hm_vhalfact','hm_vhalfinact','hm_recovfrominact', 'hm_taudeact_fast', 
        ...
        'pamscore', 'aasimilaritymat', "lqt2_dist",'lqt2_dist_sequence', 'lqt2_dist_observed', 'lqt2_dist_max', 'lqt2_dist_obs_mean','lqt2_dist_obs_max', 'lqt2_dist_obs_mean', "revel_score", 'p_mean_w')
...
tmp<-herg.combined.data[!is.na(herg.combined.data$provean_score) & !is.na(herg.combined.data$revel_score) & !is.na(herg.combined.data$pamscore),]
...
lqt2_dist <- na.omit(herg.combined.data$lqt2_dist)
...
results.2.data<-herg.combined.data[!is.na(herg.combined.data$ht_tailPeak),]
...
foo <- boot(results.2.data, function(data,indices)
  weightedCorr(results.2.data$p_mean_w[indices],results.2.data$penetrance_lqt2[indices], method="pearson", weights = results.2.data$weight[indices])^2, R=1000)
print("EM estimated LQT2 diagnosis probability versus observed cohort LQT2 diagnosis probability")
foo$t0

quantile(foo$t,c(0.025,0.975), na.rm = T)

model <- lm(penetrance_lqt2~ht_tailPeak, data = results.2.data, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
...
foo$t0
quantile(foo$t,c(0.025,0.975))
model<-lm(penetrance_lqt2~lqt2_dist, data = results.2.data, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
...
foo$t0
quantile(foo$t,c(0.025,0.975))
model <- lm(penetrance_lqt2~revel_score, data = results.2.data, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
print("REVEL score versus observed cohort LQT2 diagnosis probability")
...
tmp2<-herg.combined.data[!is.na(herg.combined.data$provean_score) & !is.na(herg.combined.data$penetrance_lqt2) & !is.na(herg.combined.data$lqt2_dist) & !is.na(herg.combined.data$revel_score),]
foo <- boot(tmp2, function(data,indices)
  weightedCorr(tmp2$p_mean_w[indices],tmp2$penetrance_lqt2[indices], method="pearson", weights = tmp2$weight[indices])^2, R=1000)
print("EM estimated LQT2 diagnosis probability versus observed literature LQT2 diagnosis probability")
foo$t0
quantile(foo$t,c(0.025,0.975))
model <- lm(penetrance_lqt2~lqt2_dist, data = tmp2, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
...
foo$t0
quantile(foo$t,c(0.025,0.975))
model <- lm(penetrance_lqt2~revel_score, data = tmp2, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
...
quantile(foo$t,c(0.025,0.975))
# Evaluate only variants with Heterozygous peak tail current measured.
tmp3<-herg.combined.data[!is.na(herg.combined.data$ht_tailPeak) & !is.na(herg.combined.data$provean_score) & !is.na(herg.combined.data$penetrance_lqt2) & !is.nan(herg.combined.data$penetrance_lqt2) & !is.na(herg.combined.data$lqt2_dist),]
foo <- boot(tmp3, function(data,indices)
  weightedCorr(tmp3$p_mean_w[indices],tmp3$penetrance_lqt2[indices], method="pearson", weights = tmp3$weight[indices])^2, R=1000)
print("EM estimated LQT2 diagnosis probability versus observed literature LQT2 diagnosis probability")
foo$t0
quantile(foo$t,c(0.025,0.975))
lqt2_dist <- 
  model <- lm(penetrance_lqt2~lqt2_dist, data = tmp3, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
print("LQT2 probability density versus observed literature LQT2 diagnosis probability")
foo$t0
quantile(foo$t,c(0.025,0.975))
model <- lm(penetrance_lqt2~ht_tailPeak, data = tmp3, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
...
foo$t0
quantile(foo$t,c(0.025,0.975))
model <- lm(penetrance_lqt2~revel_score, data = tmp3, weights = weight)
mod<-data.frame(model$fitted.values,model$model$penetrance_lqt2,model$weights)
foo <- boot(mod, function(data,indices)
  weightedCorr(mod$model.fitted.values[indices],mod$model.model.penetrance_lqt2[indices], method="pearson", weights = mod$model.weights[indices])^2, R=1000)
print("REVEL versus observed literature LQT2 diagnosis probability")
...
modfunc4 <- glm(lqt2_patho~RMSF, data = fglm, family = 'binomial')
modfunc5 <- glm(lqt2_patho~lqt2_dist, data = fglm, family = 'binomial')
modfunc6 <- glm(lqt2_patho~p_mean_w, data = fglm, family = 'binomial')
modfunc7 <- glm(lqt2_patho~lqt2_dist_sequence+lqt2_dist_observed, data = fglm, family = 'binomial')
...
names<- c("pph2_prob:", "revel_score:", "blast_pssm:", "RMSF:", "lqt2_dist:", "p_mean_w:", "combined_dist:")
...
df <- data.frame(fglm$pph2_prob, fglm$revel_score, fglm$blast_pssm, fglm$RMSF, fglm$lqt2_dist, fglm$p_mean_w,fglm$lqt2_dist_sequence+fglm$lqt2_dist_observed)
...
## Observed cohort LQT2 diagnosis probability with single observation variants.
## ROC Plot
...
n1<- herg.combined.data[herg.combined.data$total_carriers==1,]
herg.combined.data<- herg.combined.data[herg.combined.data$total_carriers>1,]
f<-data.frame(resnum=NA,lqt2_dist=NA, lqt2_dist_weight=NA)
i<-0
for(rec in seq(1,1149,1)){
  i <- i+1
  f[rec, c('lqt2_dist', 'lqt2_dist_weight')] <- funcdist(rec, "blaa", herg.combined.data, herg.combined.data$h2dist, "penetrance_lqt2", "sigmoid", 7)
  f[i, "resnum"] <- rec
  # f[i, "lqt2_dist"] <- dist
...
n1$lqt2_dist.x[!is.na(n1$lqt2_dist.y)] <-
  n1$lqt2_dist.y[!is.na(n1$lqt2_dist.y)]
n1$lqt2_dist_weight.x[!is.na(n1$lqt2_dist_weight.y)] <-
  n1$lqt2_dist_weight.y[!is.na(n1$lqt2_dist_weight.y)]
...
fglm<-n1[!is.na(n1$blast_pssm) & n1$total_carriers == 1, ]
...
modfunc3 <- glm(lqt2_patho~blast_pssm, data = fglm, family = 'binomial')
modfunc4 <- glm(lqt2_patho~RMSF, data = fglm, family = 'binomial')
modfunc5 <- glm(lqt2_patho~lqt2_dist.x, data = fglm, family = 'binomial')
modfunc6 <- glm(lqt2_patho~p_mean_w, data = fglm, family = 'binomial')
...
names<- c("pph2_prob:", "revel_score:", "blast_pssm:","RMSF:", "lqt2_dist:", "p_mean_w:")
#added for legend
auc <- round(auc, digits = 2)
leg.data <-data.frame(names,auc)
leg.data$together <- paste(leg.data$names, leg.data$auc)
...
#Code for func dist on penetrance_lqt2 This has alredy been run and the output saved as an RData to be loaded elsewhere
...
# for(rec in 1:nrow(cln.d)){
#   cln.d[rec,
#         c("lqt2_dist", "lqt2_dist_weight_all", "resnum")] <- c(funcdist(cln.d[rec, "resnum"], cln.d[rec, "var"], cln.d, h2dist, "penetrance_lqt2", "sigmoid", 7,'V8'), cln.d[rec, "resnum"])
# }
# 
# save(cln.d, file="testing.seq.Rdata")


#  #new from Brett n=1 obs
# f<-data.frame(resnum=NA,lqt2_dist=NA, lqt2_dist_weight)
# i<-0
# for(rec in seq(1,1149,1)){
#   i <- i+1
#   c(dist, dist_weight) <- funcdist(rec, "blaa", cln.d, cln.d$h2dist, "penetrance_lqt2", "sigmoid", 7)
#   f[i, "resnum"] <- rec
#   f[i, "lqt2_dist"] <- dist
#   f[i, "lqt2_dist_weight"] <- dist_weight
# }
...
>>>>>>> 416b08283fd3dc3bdf685121083abfafcbceb8d7
