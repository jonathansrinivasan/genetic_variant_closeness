...
funcdist <- function(resnum, var, deval, distances, function_name, weight_function, a, colname){
  #resnum<-toString(resnum)
  sum_weights <- 0
  sum_function <- 0
  # dont have data for the residue number
  # calculates based on hypothetical distance and sequence 
  if (is.na(match(resnum, distances$resnum))) {
  ...
  neighbors<-distances[distances$resnum==resnum, colname]
  dists<-distances[distances$resnum==resnum, colname]
  i = 0
  ...
  # estimated penetrance / weighting function
  return(c(sum_function/sum_weights, sum_weights))
}

#Edit so sequences all data 
funcdist2 <- function(resnum, var, deval, distances, function_name, weight_function, a, colname){
  #resnum<-toString(resnum)
  sum_weights <- 0
  sum_function <- 0
  distances$resnum <- NA
